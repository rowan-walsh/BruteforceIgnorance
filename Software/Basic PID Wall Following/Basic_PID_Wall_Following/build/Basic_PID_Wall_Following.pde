...
void loop()
{
        int servo = knob(VALUE_ADJUST_KNOB);
        if (servo > 180) servo = 180;
      
        RCServo0.write(servo) ;
        RCServo1.write(servo) ;
        RCServo2.write(servo) ;
...
{
	//if (leftDetected && rightDetected) error = TOO_CLOSE;
	//else if (!leftDetected && !rightDetected) error = TOO_FAR;
	//else if (leftDetected || rightDetected) error = 0;
	
	if (leftDetected) error = TOO_CLOSE;
	else error = TOO_FAR;
